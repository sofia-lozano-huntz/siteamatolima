...
function MonogramLarge() {
  return (
    <div className="grid h-24 w-24 place-items-center rounded-full border border-white/30 bg-white/10 text-2xl font-semibold tracking-[0.22em] text-white backdrop-blur-sm md:h-28 md:w-28">
      AL
    </div>
  );
}
...
export default function IntroBrand() {
  ...
  useGSAP(
    () => {
      ...
      tl.set([".intro-stage", ".brand-center", ".hero-reveal"], {
        visibility: "visible"
      });
      ...
      tl.fromTo(
        ...
        .to(".brand-center", { y: -4, duration: 0.35 })
        .to(".brand-center", {
          scale: 0.46,
          x: () => {
            const vw = window.innerWidth;
            return vw < 768 ? -112 : -360;
          },
          y: () => {
        ...
        .to(".intro-overlay", { autoAlpha: 0, duration: 0.9 }, "-=0.8")
        .to(".hero-reveal", { autoAlpha: 1, y: 0, duration: 1.05 }, "-=0.65")
        .to(".intro-stage", { pointerEvents: "none", duration: 0.01 }, "-=0.2");
    },
    { scope: root }
  );
  ...
  return (
    <div ref={root} className="relative min-h-screen">
      ...
      <div className="intro-stage texture-travertine fixed inset-0 z-20 visible">
        <div className="intro-overlay absolute inset-0 bg-[var(--overlay)]" />

        <div className="brand-center invisible absolute left-1/2 top-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-5 text-white">
          <MonogramLarge />
      ...
      <div className="hero-reveal invisible translate-y-8">
